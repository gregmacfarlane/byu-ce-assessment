...
#| fig-cap: Number of advisement appointments by class standing for CE students enrolled in the 2025-26 academic year.
#| 
# get list of CE student netID from enrollment 
ce_students <- read_xlsx("data/advisement/CCE Winter 2026 Enrollment.xlsx") |>
  filter(`Major Code` == "392850") |>
  select(name = `Student Name`, netid = `Net ID`, class = `Class  Standing`, hours = `Total BYU Class Hours`) |>
  mutate(
    # put class in a factor ordered by class standing
    class = factor(class, levels = c("Freshman", "Sophomore", "Junior", "Senior")),
    hours = cut(hours, breaks = c(-Inf, 20, 40, 60, 100, Inf), labels = c("Less than 20", "20-40", "40-60", "60-100", "More than 100"))
  )
...
appointments <- read_csv("data/advisement/Advising Appointments May 2025-May 2026.csv") |>
  # extract email addresses from title
    mutate(
      netid = str_extract(Title, "[[:alnum:]._%+-]+(?=@)"),
      name = Title |>
       str_remove("^Booked:\\s*") |>
       str_remove("\\s+[[:alnum:]._%+-]+@.*$") |>
       str_squish()
    ) |>
    select(Title, netid, name)

ce_appointments  <- appointments |>
  inner_join(ce_students, by = "netid") 

ggplot(ce_appointments, aes(x = hours)) +
  geom_bar() + xlab("BYU Credit Hours") + ylab("Number of Advising Appointments") 
```

From May 2025 to May 2026, the department advisor held `r nrow(ce_appointments)` with enrolled CE majors.
@fig-advisement shows the number of advisement appointments by class standing for CE students enrolled in the 2025-26 academic year.
Overall, more students would likely benefit from meeting with the advisor earlier in their career. The program should
develop incentives or other strategies to encourage students to meet with the advisor early.


:::{.callout-note title="Assessment Practices" collapse="true"}
The department academic advisor should provide an annual summary of student
